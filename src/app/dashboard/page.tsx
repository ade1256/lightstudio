...
const inputCls = "h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100";
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-800">
      <div className="grid min-h-screen lg:grid-cols-[260px_1fr]">
        <aside className="border-r border-slate-200 bg-slate-900 px-5 py-6 text-slate-100">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">TailAdmin style</p>
          <h1 className="mt-2 text-2xl font-semibold">Lightstudio Panel</h1>
          <nav className="mt-8 space-y-2 text-sm">
            <a href="#access" className="block rounded-md bg-slate-800 px-3 py-2">Access & Seed</a>
            <a href="#portfolio" className="block rounded-md px-3 py-2 hover:bg-slate-800">Portfolio</a>
            <a href="#pricing" className="block rounded-md px-3 py-2 hover:bg-slate-800">Pricing</a>
          </nav>
        </aside>
        ...
        <section className="px-5 py-6 sm:px-8">
          <div className="mx-auto max-w-7xl space-y-6">
            <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-2xl font-semibold text-slate-900">Dashboard Konten</h2>
              <p className="mt-1 text-sm text-slate-500">Template layout mengikuti gaya TailAdmin (sidebar + cards + clean table-like rows).</p>
            </header>

            <section id="access" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <h3 className="text-base font-semibold">Akses dan Seeder</h3>
              <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto]">
                <input value={cmsKey} onChange={(e) => setCmsKey(e.target.value)} onBlur={() => localStorage.setItem("cms-key", cmsKey)} placeholder="CMS_ACCESS_KEY" className={inputCls} />
                <button onClick={seedAllToSanity} className="h-10 rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700">{seeding ? "Seeding..." : "Seed Semua Data"}</button>
              </div>
            </section>

            <section id="portfolio" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-semibold">Manajemen Portfolio</h3>
              ...
              <form onSubmit={savePortfolio} className="grid gap-3 md:grid-cols-6">
                ...
                <button className="h-10 rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">{portfolioEditId ? "Update" : "Tambah"}</button>
              </form>

              <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
                <input type="file" accept="image/*" className={`${inputCls} file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-1 file:text-sm`} onChange={async (e) => {
              ...
              {portfolioForm.imageUrl ? <div className="mt-3 overflow-hidden rounded-lg border border-slate-200 p-2"><Image src={portfolioForm.imageUrl} alt="Preview" width={960} height={320} className="h-40 w-full rounded object-cover" /></div> : null}

              <div className="mt-4 overflow-hidden rounded-lg border border-slate-200">
                {portfolioList.map((item) => (
                  <div key={item._id} className="flex items-center justify-between border-b border-slate-100 bg-white px-4 py-3 text-sm last:border-b-0">
            ...
            <section id="pricing" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-semibold">Manajemen Paket Harga</h3>
              ...
              <form onSubmit={savePricing} className="space-y-4">
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <button className="h-10 rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">{pricingEditId ? "Update Kategori" : "Tambah Kategori"}</button>
                </div>
                  ...
                  {pricingForm.items.map((item, itemIndex) => (
                    <div key={`item-${itemIndex}`} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                      <div className="grid gap-2 md:grid-cols-[1fr_220px_auto]">
                        ...
                        <button type="button" onClick={() => setPricingForm((prev) => ({ ...prev, items: prev.items.filter((_, idx) => idx !== itemIndex) || [emptyPricingItem()] }))} className="h-10 rounded-lg border border-red-200 bg-white px-3 text-sm text-red-600">Hapus paket</button>
                      </div>
                      ...
                      <div className="mt-3 space-y-2">
                        {item.notes.map((note, noteIndex) => (
                          <div key={`note-${itemIndex}-${noteIndex}`} className="grid gap-2 md:grid-cols-[1fr_auto]">
                            ...
                            <button type="button" onClick={() => setPricingItem(itemIndex, { notes: item.notes.filter((_, idx) => idx !== noteIndex) || [""] })} className="h-10 rounded-lg border border-red-200 bg-white px-3 text-sm text-red-600">Hapus detail</button>
                          </div>
                        ))}

                        <button type="button" onClick={() => setPricingItem(itemIndex, { notes: [...item.notes, ""] })} className="h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm">+ Tambah detail</button>
                      </div>
                    </div>
                  ))}

                  <button type="button" onClick={() => setPricingForm((prev) => ({ ...prev, items: [...prev.items, emptyPricingItem()] }))} className="h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm">+ Tambah paket baru</button>
                </div>
              </form>

              <div className="mt-4 overflow-hidden rounded-lg border border-slate-200">
                {pricingList.map((category) => (
                  <div key={category._id} className="flex items-center justify-between border-b border-slate-100 bg-white px-4 py-3 text-sm last:border-b-0">
                    <p>{category.category} · {category.items?.length || 0} paket</p>
                    <div className="space-x-4">
                      <button onClick={() => {
